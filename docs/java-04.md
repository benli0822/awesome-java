...
[银行家算法](https://www.jianshu.com/p/5d01e5db5eaf)
...
本例中采取了对比hash值得方式对两个锁对象资源进行了排序操作，衍生的逻辑是对资源获取顺序的编排。进而避免产生死锁现象。
...
银行家算法，将操作系统看做是银行，而操作系统中的资源看成银行管理的资金。类比贷款流程和进程向操作系统请求分配资源的过程。

原则上：

1. 客户请求的贷款最大需求量不超过现有银行资金限额。
2. 基于原则1，客户可以分期，但是总贷款量不超过最大需求量。
3. 当银行资金池无法满足客户当前需求是，可以推迟贷款请求，但是总能保证客户在有限时间内获得贷款。
4. 客户得到所需资金后，必须能够在有限时间内归还所有资金。
